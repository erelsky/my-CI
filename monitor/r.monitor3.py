...
def commit_observe():
    ...
    while True:
        ...
        if os.path.isfile(".commit_hash"):
            try:
                response = helpers.communicate(forwarder_host, int(forwarder_port), "stats")
            except socket.error as e:
                raise Exception("can't talk to to forwarder server: %s" % e)
            if response == "OK":
                commit = ""
                with open(".commit_hash", "r") as f:
                    commit = f.readline()
                response = helpers.communicate(forwarder_host, int(forwarder_post), "forward:%s" % commit)
