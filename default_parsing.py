...
def dispatch_into_tsts(server, commit_hash):
    while True:
        print("trying to forward the executor..."):
            for runner in server.runners:
                response = server.communicate(runner["port"], int(runner["port"]), "runtest:%s" % commit_hash)
                if response == "OK":
    ...
    def handle(self):
        ...
        if command == "status":
            ...
            if not self.server.runners:
                self.request.sendall("no runners are registred yet")
            else:
                self.request.sendall("OK")
...
def serve():
    ...
    def test_checker(server):
        def namage_commit_lists(runner):
            for commit, assinged_runner in server.forwarded_commits.interitems():
                if assinged_runner == runner: 
                    del server.forwarded_commits[commit]
                    ting_commits.append(commit)
                    break
